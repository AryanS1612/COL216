...
struct SaturatingBHRBranchPredictor : public BranchPredictor {
    ...
    bool predict(uint32_t pc) {
        ...
        bool flag;
        std::bitset<2> set = table[val];
        if(!(set[1]^set[0])) (set[1]) ? flag =  true : flag = false; return flag; // strong accepting state
        if(set[1]^set[0]){
            uint32_t temp = 2*bhr[0] + bhr[1];
            set = bhrTable[temp];
            if(!(set[0]^set[1])) (set[1]) ? flag = true : flag = false; return flag; // strong accepting state
            if(set[1]^set[0]){
                temp = temp << 14;
                temp += val;
